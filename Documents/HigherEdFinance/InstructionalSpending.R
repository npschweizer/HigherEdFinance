...
bp <-
  ggplot(dfPrivate,
         aes(
           x = "",
           y = "count",
           fill = dfPrivate$SpendingExceedsTuition
         ))
bp + geom_bar(width = 1, stat = "identity") + coord_polar("y", start = 0)
...
Private = ggplot(
  dfPrivate,
  aes(
    y = dfPrivate$SpendingPerStudent,
    x = dfPrivate$RevenuePerStudent
  )
)
Private + geom_point() + labs(y = "Instructional Spending Per Student", x = "Revenue from Tuition and Fees Per Student") #+
  #scale_x_discrete(breaks = dfPrivate$Revenue.from.tuition.and.fees[c(T,F,F,F,F,F,F,F,F,F,F,F)]) +
  #theme(axis.text.x = element_text(angle = 90, hjust = 1))
g = ggplot(dfPrivate, aes(dfPrivate$Revenue.from.tuition.and.fees..minus.spending.on.instruction.per.student))
g + geom_boxplot()